# 📂 DAFTAR VERSI & SPESIFIKASI FILE
**Model**: Qwen3.5-4B-Uncensored-HauhauCS-Aggressive
**Sumber Asli**: https://huggingface.co/HauhauCS/Qwen3.5-4B-Uncensored-HauhauCS-Aggressive

---

## 📊 PERBANDINGAN SEMUA VERSI GGUF

| Nama File | Ukuran | Kualitas | Detail | Cocok Untuk |
|---|---|---|---|---|
| **BF16.gguf** | **7,9 GB** | 💯 **100% MURNI** | Tidak ada penurunan sama sekali. Akurasi tertinggi, teknis paling lengkap. | ✅ **REKOMENDASI UTAMA**<br>PC/Laptop RAM ≥12GB<br>*(Cocok untuk Axioo Hype 5 16GB)* |
| **Q8_0.gguf** | **4,2 GB** | ⭐ 98–99% | Hampir sama persis, beda sangat kecil. Sangat stabil. | PC/Laptop RAM ≥6GB<br>Ingin hemat sedikit ruang |
| **Q6_K.gguf** | **3,3 GB** | ⭐ 96–97% | Kualitas sangat baik, detail masih lengkap. | RAM ≥5GB, ingin ringan |
| **Q5_K_M.gguf** | **2,8 GB** | ⭐ 94–95% | Seimbang ukuran & kualitas. | RAM ≥4GB, perangkat menengah |
| **Q4_K_M.gguf** | **2,3 GB** | ✅ 90–92% | Standar, cukup bagus, ringan. | RAM ≥3GB, HP/perangkat kecil |
| **Q2_K.gguf** | **1,3 GB** | ⚠️ 75–80% | Agak turun, kurang rinci. | Hanya untuk spesifikasi sangat rendah |


---

## 🧠 SPESIFIKASI TEKNIS MODEL
- **Arsitektur**: Hibrida Gated DeltaNet + Full Attention
- **Parameter**: 4 Miliar (4B)
- **Lapisan**: 32 Lapisan Decoder
- **Konteks Maksimal**: 262.144 Token (~200.000 kata)
- **Multimodal**: Mendukung Gambar & Video (tetap aktif penuh)
- **Bahasa**: 201 Bahasa (Indonesia sangat baik)
- **Metode Modifikasi**: Abliterasi Agresif → Hapus aturan tanpa rusak kecerdasan
- **Divergensi Bobot**: 0,0217 (hanya 2,17% berubah)

---

## ✅ REKOMENDASI PILIHAN

> **Untuk Axioo Hype 5 (Ryzen 5 6600H / 16GB DDR5):**

> ✅ **Pilihan standar pakai: BF16.gguf**

> ✅ Alasan: RAM cukup besar, grafis kuat, hasil paling akurat & rinci.

> ❌ **JANGAN pakai Q4 atau lebih kecil**: kemampuan laptop terbuang.
